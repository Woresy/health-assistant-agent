...
飞书提醒确认后只写入计划任务，由本地调度器到期发送；主动 check-in 仍复用这一个
Tool，因此清单保持 15 个。Webhook 和签名密钥永远不进入 Tool Schema。

记忆查看、逐条遗忘、导出和全部清除不属于模型 Tool。它们由“数据与隐私”页直接
调用受信任服务，删除类操作二次确认，因此公开给模型的 Tool 数仍保持 15 个。
